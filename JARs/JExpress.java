 ...
 * 
 * Compile the application with : javac JExpress.java Run the application with : java JExpress Get
 * the documentation with : javadoc -d ../doc JExpress.java
 */
...
public class JExpress {
  ...
  /**
   * A callback called to process a HTTP request in order to create a HTTP response.
   */
  @FunctionalInterface
  ...
  private static Function<String, Optional<Map<String, String>>> matcher(String uri) {
    ...
    BiConsumer<String[], Map<String, String>> consumer = (_1, _2) ->
    {
      /* empty */ };
  ...
  private void method(String method, String path, Callback callback) {
    ...
    this.pipeline = exchange ->
    {
      ...
      Optional<Map<String, String>> paramsOpt = Optional
        .of(exchange)
  ...
  public static void main(String[] args) throws IOException {
    if (args.length != 2) {
      System.err.println("Usage: java JExpress port numberOfRedirection");
      return;
    }
    ...
    var n = Integer.valueOf(args[1]);

    app.get("/", (req, res) ->
    {
      var body = "<html> It's " + counter.counter + " redirection</html>";
      if (counter.counter < n) {
        ...
        res.status(200).type("text/html", "utf-8").send("helloooooo");
      }
    ...
    out.println("application started on port " + port);
  }
}
